...
int index_of_alphabet(const string& str) {
    ...
    auto it = find(alphabet.begin(), alphabet.end(), str);
...
int main() {
    ...
    if (unordered_set<int>(origin_int.begin(), origin_int.end()).size() < 26 ||
        unordered_set<int>(encrypt_int.begin(), encrypt_int.end()).size() < 26) {
        if_fail = true;
    } else {
        for (int i = 0; i < n; ++i) {
    ...
    if (if_fail) {
        ...
        for (int i = 0; i < m; ++i) {
            cout << (char)(code[translate_int[i]] + 'A' - 1);
        }
        cout << endl;
    }
