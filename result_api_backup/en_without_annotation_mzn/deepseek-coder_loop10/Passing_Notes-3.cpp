...
int main() {
    ...
    for (int i = 1; i < m; ++i) dp1[i][0] = dp1[i-1][0] + students[i][0];
    for (int j = 1; j < n; ++j) dp1[0][j] = dp1[0][j-1] + students[0][j];
    for (int i = 1; i < m; ++i) {
        for (int j = 1; j < n; ++j) {
    ...
    for (int i = m-2; i >= 0; --i) dp2[i][n-1] = dp2[i+1][n-1] + students[i][n-1];
    for (int j = n-2; j >= 0; --j) dp2[m-1][j] = dp2[m-1][j+1] + students[m-1][j];
    for (int i = m-2; i >= 0; --i) {
        for (int j = n-2; j >= 0; --j) {
    ...
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            int value = dp1[i][j] + dp2[i][j] - students[i][j];
