...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        tap[i + 1] = 0;
        d[i] = -Y[i]; // Default move is -Y[i]
        
        // Check if we can improve the move by using X[i]
        if (X[i] > 0) {
            int new_d = X[i] * tap[i + 1];
            if (new_d > d[i]) {
                d[i] = new_d;
            }
        }
        ...
        bird[i + 1] = bird[i] + d[i];
        if (bird[i + 1] >= m) bird[i + 1] = m;
        
        // Check if the bird passes through any tube
