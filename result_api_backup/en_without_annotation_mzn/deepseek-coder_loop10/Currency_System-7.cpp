...
int main() {
    ...
    for (int i = 0; i < T; ++i) {
        ...
        for (int x : sub_a) express_a[x] = true;
        for (int x : sub_b) express_b[x] = true;

        for (int j = 1; j <= max_n[i]; ++j) {
            if (express_a[j]) {
                for (int x : sub_a) {
                    if (j + x <= max_n[i]) express_a[j + x] = true;
                }
            }
            if (express_b[j]) {
                for (int x : sub_b) {
                    if (j + x <= max_n[i]) express_b[j + x] = true;
                }
            }
        }
    ...
    int result = accumulate(m.begin(), m.end(), 0);
    cout << result << endl;
