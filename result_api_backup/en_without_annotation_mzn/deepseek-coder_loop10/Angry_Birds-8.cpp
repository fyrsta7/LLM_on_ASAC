...
bool is_line_fit(const vector<Point>& points, float a, float b) {
...
int main() {
    ...
    for (int t = 0; t < T; ++t) {
        for (int i = split_n[t]; i < split_n[t] + n[t]; ++i) {
            bool found = false;
            for (float a = -10.0; a < 0.0 && !found; a += 0.1) {
                for (float b = -10.0; b <= 10.0 && !found; b += 0.1) {
                    if (is_line_fit(vector<Point>(points.begin() + split_n[t], points.begin() + split_n[t] + n[t]), a, b)) {
                        found = true;
                        ++answer[t];
                    }
                }
