...
void build_tree(int left, int right, int idx) {
    if (left == right) {
        ...
        int left_count_0 = 0, left_count_1 = 0;
        for (int i = left; i <= mid; ++i) {
            if (number[i] == 0) left_count_0++;
            else left_count_1++;
        }
        int right_count_0 = 0, right_count_1 = 0;
        for (int i = mid + 1; i <= right; ++i) {
            if (number[i] == 0) right_count_0++;
            else right_count_1++;
        }
        if (left_count_0 == 0 && right_count_0 == 0) tree[idx] = 0;
        else if (left_count_1 == 0 && right_count_1 == 0) tree[idx] = 1;
        else tree[idx] = 2;
