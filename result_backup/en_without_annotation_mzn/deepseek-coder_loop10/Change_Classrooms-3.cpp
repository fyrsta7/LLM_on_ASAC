...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= m; ++j) {
            ...
            if (j > 0) {
                dp[i][j] = min(dp[i][j], dp[i - 1][j - 1] + distD[i - 1][C[i]] * K[i - 1] + dist[i - 1][C[i]] * (1 - K[i - 1]));
            }

            if (j > 0) {
                dp[i][j] = min(dp[i][j], dp[i - 1][j - 1] + dist[i - 1][D[i]] * K[i - 1] + distD[i - 1][D[i]] * (1 - K[i - 1]));
            }

            if (j > 1) {
                dp[i][j] = min(dp[i][j], dp[i - 1][j - 2] + distD[i - 1][D[i]] * K[i - 1] * K[i] + distD[i - 1][C[i]] * K[i - 1] * (1 - K[i]) + dist[i - 1][D[i]] * (1 - K[i - 1]) * K[i] + dist[i - 1][C[i]] * (1 - K[i - 1]) * (1 - K[i]));
            }
        }
    }
