    ...
    for (int i = 0; i < plan_num; ++i) {
        for (int j = 0; j < symbol_count; ++j) {
            ...
            if (expr_int[symbol_loc[t]] == 1) {
                state[i][j + 1][t] = (state[i][j][t] == 0 && state[i][j][t + 1] == 0) ? 0 : 1;
                state[i][j + 1][t + 1] = (state[i][j][t] == 0 && state[i][j][t + 1] == 0) ? 0 : 1;
            } else {
                state[i][j + 1][t] = (state[i][j][t] == 1 && state[i][j][t + 1] == 1) ? 1 : 0;
                state[i][j + 1][t + 1] = (state[i][j][t] == 1 && state[i][j][t + 1] == 1) ? 1 : 0;
            }
