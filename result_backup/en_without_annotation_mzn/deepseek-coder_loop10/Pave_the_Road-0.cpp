...
int main() {
    ...
    for (int i = 0; i < n; ++i) state[0][i] = d[i];

    int actions = 0;
    for (int i = 0; i < n; ++i) {
        while (state[actions][i] > 0) {
            int left = i, right = i;
            while (right < n && state[actions][right] > 0) ++right;
            --right;
            ++actions;
            left_right[actions] = {left + 1, right + 1};
            for (int j = left; j <= right; ++j) --state[actions][j];
            for (int j = 0; j < n; ++j) state[actions][j] += state[actions - 1][j];
        }
    }
