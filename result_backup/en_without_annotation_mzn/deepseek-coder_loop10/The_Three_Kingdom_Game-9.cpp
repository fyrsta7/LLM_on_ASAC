...
int find_max(const vector<int>& my_set, const vector<int>& free_set) {
    ...
    for (int my_pointer : my_set) {
        for (int free_pointer_candidate : free_set) {
            if (value[my_pointer][free_pointer_candidate] > max_val) {
                max_val = value[my_pointer][free_pointer_candidate];
                free_pointer = free_pointer_candidate;
            }
        }
    }
    return free_pointer;
